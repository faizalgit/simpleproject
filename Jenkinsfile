...
node{
	...
	stage("deploy"){
		...
		sshagent(['tomcatID']) {
			...
			sh "ssh tomcat@${env.targetServer} cd ${deployPath} && pwd && curl http://104.196.30.112:8081/repository/maven-nexus-repo/com/simpleproject/simpleproject/v1/simpleproject-v1.war"
